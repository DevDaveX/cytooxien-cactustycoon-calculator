...
const Calculator = () => {
  ...
  const availableBuildings = useMemo(() => {
    return buildings.filter(
      b => b.status === 'An' && BUILDING_LABELS[b.type] !== "Kaktuslager"
    );
  }, [buildings]);
  ...
  const handleAddBooster = () => {
    ...
    const newBooster: Booster = { 
      ...
      deviceLabel: `${BUILDING_LABELS[targetBuilding.type] || targetBuilding.type} (Lvl ${targetBuilding.level})`,
      amount, 
      expiresAt 
    };
    
    setBoosters(prev => {
      const otherBoosters = prev.filter(b => b.buildingId !== selectedBuildingId);
      const updated = [...otherBoosters, newBooster];
      saveBoosters(updated);
      return updated;
    });

    toast({ title: "🚀 Booster aktiv", description: `${newBooster.deviceLabel} wird geboostet!` });
    ...
    const interval = setInterval(() => {
      ...
      setBoosters(prev => {
        ...
        if (filtered.length !== prev.length) {
          localStorage.setItem(COOKIE_BOOSTER_KEY, JSON.stringify(filtered));
          return filtered;
        }
        return prev;
      });
  ...
  useEffect(() => {
    ...
    const savedConfig = getCookie(COOKIE_CONFIG_KEY);
    if (savedConfig) {
      const imported = importConfig(savedConfig);
      if (imported && imported.length > 0) setAllBuildings(imported);
    }
  ...
  const handleShareLink = async () => {
    const code = exportConfig(buildings);
    const url = `${window.location.origin}/rechner?config=${code}`;
    try {
      ...
      <div className="px-4 py-6 space-y-6 max-w-6xl mx-auto">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3 text-left">
            ...
            <div>
              <h1 className="font-title text-sm text-cactus pixel-text tracking-tight">Kaktus Cytooxien Rechner</h1>
              <p className="text-muted-foreground text-[10px] mt-1 uppercase tracking-widest leading-none">Dashboard</p>
            </div>
          </div>
        ...
        {boosters.length > 0 && (
          <div className="mc-panel p-4 border-l-4 border-orange-500 bg-orange-500/10 shadow-lg">
            <h3 className="font-title text-[10px] mb-3 uppercase tracking-widest text-orange-400 text-left font-bold">🔥 Aktive Maschinen-Booster</h3>
            ...
            {Object.entries(categories).map(([cat, types]) => {
              ...
              return (
                <div key={cat} className="space-y-2">
                  <h3 className="text-muted-foreground text-xs font-title uppercase tracking-[0.2em] opacity-50">{cat}</h3>
                      ...
                      return (
                        <div key={type} className="mc-panel !p-3">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <div>
                              <h4 className="font-bold text-sm pixel-text leading-none mb-1 uppercase">{BUILDING_LABELS[type]}</h4>
                              <span className="text-xs text-cactus bg-cactus/10 px-1 rounded">{g.active}/{g.total} aktiv</span>
                            </div>
                          </div>
                  ...
                  >
                    {availableBuildings.map(b => (
                      <option key={b.id} value={b.id} className="bg-zinc-900">
                        {BUILDING_LABELS[b.type] || b.type} (Lvl {b.level})
                      </option>
                    ))}
                  </select>
                </div>
